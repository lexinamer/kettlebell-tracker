<template>
  <BaseDrawer 
    :isOpen="isOpen" 
    :title="editMode ? 'Edit Workout' : 'New Workout'"
    @close="$emit('close')"
    @save="handleSubmit"
  >
...
watch(() => props.workout, (newWorkout) => {
  if (newWorkout) {
    editMode.value = !!newWorkout.id
    formData.value = {
      ...newWorkout,
      date: newWorkout.date?.toDate 
        ? newWorkout.date.toDate().toISOString().split('T')[0]
        : newWorkout.date,
      blocks: JSON.parse(JSON.stringify(newWorkout.blocks))
    }
  } else {
